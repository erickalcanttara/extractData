...
public class ContasParaTest {
    ...
    @ParameterizedTest
    @CsvFileSource(resources = "/produtosEmTeste/produtos_com_status_conta_venc25_e200-paginas.csv", delimiter = '\t', numLinesToSkip = 1)
    void getContasParaTest(int produto, int page, String ano_mes_dia, int status_conta) throws IOException {

        String dia_vencimento = ano_mes_dia.split("-")[2];
        ...
        for (int i = 0; i < 50; i++) {
            ...
            if (data_ultimo_pagamento != null){
                ...
                if (saldoAtualFinal > 50){
                    ...
                    file.write(id_produto + "\t" + id_status_conta + "\t" + id_conta + "\t" + (random.nextInt(7) + 1) + "\t" + ano_mes_dia + "\t"
                            + saldoAtualFinal + "\n");
                    file.flush();
                    file.close();
                }
